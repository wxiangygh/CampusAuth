...
- **DNS 设置**：独立标签页（排在设置页之前）提供阿里云、北京联通、114 DNS、Google、Cloudflare 和自定义 IP 地址。分流解析使用保存的服务器；认证工作流的 IPv6 DNS 单独配置，留空使用自动获取。DNS Fallback 仍遵循本地网络的 DNS。
- **定向解析绑定**：为指定域名、IP 或网段绑定解析服务器（域名规则含子域名，最长匹配优先），仅影响本程序解析；一键下发到 Windows NRPT 后可让浏览器等所有程序按域名换用指定 DNS 服务器（需管理员权限，可在应用内清除）。
